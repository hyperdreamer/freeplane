# Graph Workspace Batch E Continuation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use the deterministic
> subagent-driven-development controller to implement this plan task-by-task.

**Goal:** Audit the preserved Task 25 canvas commit after the blocked reviewer dispatch, then implement and verify the remaining Batch E interaction and accessibility deliverables.

**Architecture:** The first task performs a fresh, read-only audit of the exact committed Task 25 range and its focused tests; it does not reroll or modify that valid commit. The next two tasks implement the original Backlog Tasks 26 and 27 sequentially, reusing the immutable geometry, viewport, projection, and paint-state contracts established by Task 25. Each task has its own focused tests, exact allowlist, independent review, and commit.
...
- This continuation covers the preserved original Backlog Task 25 range plus original Backlog Tasks 26 and 27 in order. Task 1 is a no-source-change audit of `02f02355d9a33851a8a4417c4610d1897f716a50..cd21a68a82c0914bb14f351b473e04884132284c`; it must not reroll that range. Tasks 2 and 3 are the original Tasks 26 and 27. Do not combine implementation commits.
- Preserve the prior run rooted at `.superpowers/sdd/2026-08-18-graph-workspace-batch-e`; its terminal `DISPATCH_MISMATCH_BLOCKED` state, reports, prompts, and child transcript are evidence only and must not be edited or admitted.
...
## Task 1: Audit Preserved Backlog Task 25 Canvas Commit

**Implementer tier:** Capable

**Files:**
- Read only: `02f02355d9a33851a8a4417c4610d1897f716a50..cd21a68a82c0914bb14f351b473e04884132284c`
...
- Do not modify source, tests, the index, `HEAD`, the prior run root, or any tracked file.

**Interfaces:**
- Audits the exact Task 25 implementation range from parent `02f02355d9a33851a8a4417c4610d1897f716a50` through `cd21a68a82c0914bb14f351b473e04884132284c`.
- Verifies the Task 25 brief at `ai-specs/tasks/backlog/006-implement-graph-workspace.md` and the committed continuation design at `docs/superpowers/specs/2026-08-18-graph-workspace-batch-e-continuation-design.md` against the actual range.
- Writes only the bounded implementer report path supplied by the controller. A `DONE` report means the audit found no blocker; a `DONE_WITH_CONCERNS` report must name a non-empty observational concern. Do not write a source fix.

- [ ] **Step 1: Establish the preserved range and scope**

Verify the parent and head objects with `git cat-file -t`, inspect `git diff --name-status 02f02355d9a33851a8a4417c4610d1897f716a50..cd21a68a82c0914bb14f351b473e04884132284c`, and confirm the changed paths are exactly the ten original Task 25 paths. Confirm the continuation branch has no uncommitted source or test changes before the audit begins. Do not inspect or rely on the blocked run's reviewer report.

- [ ] **Step 2: Re-read the Task 25 contract against source**

Read the Task 25 brief and inspect `GraphViewport`, `GraphCanvas`, `GraphPainter`, `GraphTheme`, `RenderingLevel`, `AdaptiveRenderingPolicy`, `GraphPaintState`, and all three Task 25 tests. Check finite viewport validation and transforms, persisted conversion, fit/reset behavior, immutable paint state, theme resolution without Swing-default mutation, exact rendering thresholds, geometry-authoritative painting, suppressed hull handling, edge attachment/arrow/multiplicity behavior, label detail rules, full-bleed painting, and absence of print/export API. Check that the tests would fail for wrong transform signs, wrong thresholds, ignored geometry radius, incorrect layer ordering, and missing high-count editability.

- [ ] **Step 3: Run the preserved focused and compatibility gates read-only**
...
Record the exact results. Do not alter tests to make the preserved commit pass. If a load-bearing defect is found, report its file and line; the controller may open a bounded fix round limited to the original ten Task 25 paths.

- [ ] **Step 4: Write the audit report**

Inspect `git status --porcelain`, `git diff --stat`, and `git diff --check` before writing the report. Report the exact range, path allowlist result, focused test result, compatibility result, and any findings. If the audit is clean, use `STATUS: DONE`, include no concern section, and state `COMMIT: cd21a68a82c0914bb14f351b473e04884132284c` as the preserved commit under audit. Do not create a new source commit.

- [ ] **Step 5: Confirm the preserved commit boundary**

Treat `cd21a68a82c0914bb14f351b473e04884132284c` as this task's commit artifact and confirm it remains the only Task 25 source commit in the range. The audit task must not create, amend, or stage a new source commit; the controller's task-complete evidence is the preserved commit SHA plus the bounded audit report.
