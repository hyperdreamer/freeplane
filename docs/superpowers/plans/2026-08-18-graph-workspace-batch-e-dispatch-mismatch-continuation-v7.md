# Graph Workspace Batch E Dispatch-Mismatch Continuation Implementation Plan V7

> **For agentic workers:** REQUIRED SUB-SKILL: Use the deterministic
> subagent-driven-development controller to execute this plan task-by-task.

**Goal:** Recover and complete the finite-coordinate edge-hit correction preserved after the blocked V6 dispatch, independently certify Task 26, and implement Backlog Task 27 keyboard traversal and accessible virtual children.

**Architecture:** Task 1 resumes from the preserved two-file dirty state left by the incomplete V6 implementer, validates and completes the finite-coordinate edge-hit correction, then receives a fresh independent review. It never rewrites existing Task 26 commits. Task 2 layers deterministic traversal and lightweight Swing accessibility over the existing immutable projection, layout, geometry, paint, and interaction values. Every task is independently reviewed before the next task or final review.
...
- The V5 run is terminal at `.superpowers/sdd/v5` with phase `TASK_BLOCKED`, and the V6 run is terminal at `.superpowers/sdd/v6` with phase `DISPATCH_MISMATCH_BLOCKED`; preserve both roots, states, reports, prompts, transcripts, and partial-child evidence byte-for-byte. Neither incomplete child result is admissible evidence. V7 intentionally starts with the two source paths documented in the recovery design dirty; inspect them in place and never reset, checkout, clean, or discard them.
...
- Read-only: `docs/superpowers/specs/2026-08-18-graph-workspace-batch-e-dispatch-mismatch-v7-recovery-design.md:1-end`
- The only source deliverable is the bounded edge-hit correction and its regression tests; no other source/test path may change. The worktree intentionally contains the incomplete V6 implementer edits in the two allowlisted paths. Inspect and finish them in place; do not reset, checkout, clean, or discard them. The current partial code already includes exponent-tagged distance logic and the extreme/mixed/subnormal regressions. Write the normal implementer report under the dispatched run root. Use targeted reads only; do not invoke CodeGraph or read predecessor run artifacts.

**Interfaces:**

- Consumes: the committed Task 26 source/test range `8d54ecda2157c06baa9b765cc92eb2a82e834506..54cab57876bb73bde13945bbbb8493ed7d34ab66`, the bounded test-only correction `8ef6d2e88043ae406a49e07aa2b0608c40c62f76`, the prior committed correction `4597825bf14cd36f3bad82fab929cdbda79323e7`, the preserved V6 dirty two-file edits, and finite near-limit geometry support.
...
Record `HEAD`, full status, the empty index, exact dirty paths, ancestry and prior range path checks, and the V6 partial diff. Confirm `HEAD` is `6291ad82fa807d4c45da04b2ecff57c935d1d50d`, only the two allowlisted source paths are dirty, and no unrelated changes exist. Do not reset, checkout, clean, stage, or commit. Record and approve the intentional preflight conflict before dispatch.
...
- [ ] **Step 3: Verify the preserved regressions and add only a missing falsifiable case**

Run the existing extreme-span, mixed-magnitude, tiny-offset, and positive-subnormal/zero-tolerance tests against the preserved implementation. If a required case is absent or does not fail against the committed V5 correction, add the smallest focused test in the same allowlisted test file and prove the red phase with a disposable predecessor copy. Do not add another numerical abstraction or broaden fixtures unnecessarily.

- [ ] **Step 4: Finish and audit the exponent-aware correction**

Inspect the preserved `GraphHitIndex` arithmetic for finite differences, cancellation, projection clamping, zero-length segments, subnormal distances, tolerance comparison, non-finite rejection, nearest-edge ordering, and ordinary behavior. Make only the minimal correction needed by fresh evidence. The implementation must not use common absolute-coordinate scaling, squared overflow, or a conversion that turns a positive subnormal distance into zero; it must not reject or clamp valid finite geometry. Then verify the existing Task 26 anchor, suppressed-hull, node-order, interaction, and projection-only search contracts by focused tests and direct reads.

- [ ] **Step 5: Run green verification and write the recovery report**

Run the focused interaction/search suite and named viewport/paint/adaptive compatibility selection with Zulu 21 using bounded logs. Confirm all finite-coordinate regressions, the anchor test, and all interaction/search tests pass. Inspect the actual two-file diff/status, delete temporary artifacts, and write the report with exact counts and commit evidence. Stop exploratory work once these gates pass.
