...
- Keep GraphStream implementation classes package-private in `org.freeplane.plugin.graph.layout.graphstream`; `GraphStreamLayoutFactory` is the only public construction seam. `TypedSpringBox` extends `org.graphstream.ui.layout.springbox.implementations.SpringBox`; `TypedNodeParticle` extends `org.graphstream.ui.layout.springbox.implementations.SpringBoxNodeParticle`.
...
- Map correction evaluates every unordered map-root pair against the original immutable hull snapshot, accumulates signed translation deltas per map, and applies each map's total delta once at the end. It skips `BoundaryTier.SUPPRESSED` roots.
...
Create `GraphStreamBoundaryShould` using the existing ArchUnit pattern (`ClassFileImporter` and `noClasses()` rules) to assert that non-`layout.graphstream` production classes do not depend on `org.graphstream..`, that no production class depends on `org.graphstream.ui.layout.LayoutRunner`, and that reflection over public layout constructors and methods finds no GraphStream parameter, return, field, or exception type. Factory-created engines expose only `LayoutEngine`. `LayoutCalibration.spikeDefaults()` must have positive strengths satisfying exactly `containment < hierarchy < sameMap`. Create `TypedForcesShould` fixtures using existing immutable projection builders and test: equal requests yield identical positions; changed workspace IDs yield distinct deterministic seeds; visible nodes and enclosure anchors appear in `LayoutPositions`; parallel projected edges do not fail; direct containment and hierarchy topology survive; 100 add/remove cycles return to baseline coverage; aggregate cross-map fan-out never moves a particle farther than `0.005`; a higher `NodeProminence.scale()` increases separation; and an active pinned neighbor remains exactly at its supplied coordinates.
...
Implement the four public layout values before GraphStream code. Validate nulls, nonnegative frame indices, immutable list/map copies, `diff.afterGeneration() == projection.generation()`, and that `projection.prominence().keySet()` exactly covers `projection.nodes()`; reject `structure()` or `resolved()` projections without a complete prominence map. The engine, not the value, owns reset semantics: `reset()` discards solver state, rebuilds from the last accepted request, reseeds deterministically, and resets the next frame index to zero. Finite-coordinate validation applies to every engine-produced `LayoutPositions` snapshot before it enters a `LayoutFrame`.

- [ ] **Step 4: Implement the GraphStream-private engine**

Create `GraphStreamLayoutFactory.create`, package-private `GraphStreamLayoutEngine implements LayoutEngine`, `TypedSpringBox extends SpringBox`, and `TypedNodeParticle extends SpringBoxNodeParticle`. Construct a `MultiGraph`, attach the sink before adding nodes or edges, retain unaffected particles on `apply`, remove obsolete particles and springs, and seed new particles with the canonical SHA-256 encoding from Global Constraints. Use one particle for each node and anchor, create relationship, direct containment, and hierarchy links from the ordered projection, set active pins exactly, and ignore dormant pins. `step()` computes one frame and snapshots exactly the projected node keys and enclosure hull keys into ordered `LayoutPositions`; it increments its index and never leaks GraphStream values. `reset()` rebuilds from the last request; `close()` releases adapter resources idempotently.
...
- Produces: `public final class LayoutWorker implements AutoCloseable { public LayoutWorker(LayoutCalibration calibration); public CompletionStage<LayoutFrame> submit(LayoutRequest request); public CompletionStage<LayoutFrame> step(); public void pause(); public void restart(); public LayoutFrame lastValidFrame(); public void close(); }`. Provide package-visible `LayoutWorker(Supplier<LayoutEngine> engineFactory, PerceptualIdlePolicy idlePolicy)` for deterministic fake-engine tests; the public constructor creates engines with `GraphStreamLayoutFactory.create(calibration)` and uses `PerceptualIdlePolicy.spikeDefaults()`.
- Extends: `LayoutFrame` with `public List<LayoutConflict> conflicts()` and `public PerceptualIdlePolicy.IdleMeasurement idle()`. Retain Task 1's raw-frame factory and add `public static LayoutFrame withDiagnostics(LayoutFrame raw, List<LayoutConflict> conflicts, PerceptualIdlePolicy.IdleMeasurement idle)` so Task 1's adapter needs no modification.

- [ ] **Step 1: Write correction, idle, and worker tests**

Create `MapTierCorrectionShould` to construct two and three map-root hull fixtures and assert deterministic pair order, additive per-map deltas computed from the original hull snapshot, exact `-T/2` for the first hull's map and `+T/2` for the second hull's map, the signed full translation when one map is rigid, no movement plus one conflict when both maps are rigid, suppression filtering for `BoundaryTier.SUPPRESSED` roots, every node and anchor in a moved map has the same delta, and no pinned node moves. Create `PerceptualIdlePolicyShould` to assert finite constructor validation, RMS/max over both nodes and anchors, streak reset when either key set changes, immediate idle for two empty frames, and the exact injectable spike default values. Create `LayoutWorkerShould` using a fake `LayoutEngine` to assert one-at-a-time serialized calls, pin/dormant/unpin transitions, raw-frame temporary geometry then correction, decorated conflicts and idle values, failure preserving `lastValidFrame`, normal pause/restart behavior, failed-engine replacement on restart, pre-submit/paused completed stages, close cancellation of queued stages, and 25 close cycles without a surviving worker thread.
...
Iterate non-suppressed map-root `ProjectedEnclosure` values in their stable projection order and process each unordered pair once against the original `GraphGeometry` hull snapshot. Accumulate a signed `LayoutPoint` delta in a deterministic `LinkedHashMap<MapReferenceId, LayoutPoint>` for each movable map; do not mutate hull geometry or apply pair corrections in place. Obtain current hulls from `GraphGeometry.hulls()`; skip pairs whose root hull is absent or whose separating translation is `(0, 0)`. For movable pairs use `T = HullIntersection.minimumSeparatingTranslation(hullA, hullB)`, which moves the second hull B clear of A. Add `-T/2` to map A and `+T/2` to map B. If exactly one map has any active pin, add only `+T` to B when A is rigid, or only `-T` to A when B is rigid. If both maps are rigid, leave their deltas unchanged and add one ordered conflict containing both maps' active pins. Apply each accumulated map delta once to exactly its node and anchor keys, preserve input ordering, and return immutable positions/conflicts.

- [ ] **Step 5: Implement the one-executor worker**

Make `LayoutWorker` own one named single-thread executor and a GraphStream-free `Supplier<LayoutEngine>`. Queue `submit` and `step` as `CompletableFuture` work on that executor. `submit` installs the request through `engine.apply`; `step` calls `engine.step`. For every successful raw frame, assert that its `LayoutPositions.nodes().keySet()` equals the current projection's node keys and its `anchors().keySet()` equals the current projection's enclosure hull keys, then compute temporary hulls with `GraphGeometryEngine`. The Task 18 `GraphGeometry` form is sufficient here: correction reads only `hulls()` and does not require Task 19's future labels map. Apply `MapTierCorrection`, measure idle against the prior corrected frame, decorate the frame with `LayoutFrame.withDiagnostics`, and update the retained last-valid frame. Catch engine or geometry failures, retain the old valid frame, and return a decorated failed frame. `pause` causes later `step` calls to return completed retained-frame stages; normal `restart` resumes without reset, while failed `restart` closes/replaces the engine before the next submitted request. Track pending futures so `close` cancels unresolved ones with `CancellationException`, closes the engine on its owner thread where possible, shuts down the executor, and rejects later work.
