# Graph Workspace Batch H Review Report Continuation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use the deterministic
> subagent-driven-development controller to implement this plan task-by-task.
>
> This successor preserves both terminal bounded-audit outcomes as immutable
> evidence and constrains the independent reviewer to a compact, report-first
> verification path. Commit `50d0c3bd1502158b19f3ea459cc6cfe5044525d0` remains
...
- The terminal runs at `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-ui-shell-continuation`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-ui-successor`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-post-task-2-mismatch`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-status-lifecycle-recovery`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-lifecycle-ui-continuation`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-lifecycle-audit-reissue`, `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-pointer-continuation`, and `.superpowers/sdd/2026-08-20-graph-workspace-batch-h-bounded-audit-continuation` are immutable audit history only. Do not reopen, modify, or cite their child reports as approval evidence.
...
- Keep child output bounded. For Gradle or other verbose commands, redirect complete stdout/stderr to files beneath the run root, inspect only exit status plus short `tail`/summary output, and do not paste full logs into the child transcript. After the final verification action, write the required report artifact as the immediate next action before any optional explanation, skill read, broad scan, or additional command; the report is the only completion evidence.
- Task 1's independent reviewer must inspect only the exact two named files and the pinned candidate range, must not use CodeGraph or broad source dumps, and must run only the focused `DefaultGraphWorkspaceControllerShould` suite. The implementer's full-suite log is an observational input; the reviewer does not rerun the full plugin suite. The reviewer must write `task-1-review-report.md` immediately after the focused result and then return its verdict.
...
Write the required report immediately after these checks. Report concrete results, the exact inspected range, and any finding. Do not alter the two source files or create a commit. The following fresh task-reviewer gate independently approves or rejects this exact range. Its bounded protocol is: inspect only the two named files and this range; do not use CodeGraph or broad source dumps; run only the focused controller suite with output redirected under the run root; then write `task-1-review-report.md` as the immediate next action and return the review verdict. Do not rerun the full plugin suite in the reviewer role.
