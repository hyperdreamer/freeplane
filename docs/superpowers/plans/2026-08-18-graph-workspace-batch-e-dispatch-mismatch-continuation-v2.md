# Graph Workspace Batch E Dispatch-Mismatch Continuation Implementation Plan V2
...
For the search probe, create a separate disposable archive from `54cab...` and make only a temporary `GraphSearchModel.java` mutation that appends `ProjectedNode.source().toString()` to each indexed safe-text value. Run the focused `GraphSearchModelShould` tests with all output redirected to a temporary log; inspect the XML result and print only a one-line status. The current fixture is a load-bearing coverage gap if the mutant still passes because its `SourceSideFixture` sentinel is not represented by any projected node source identity. Delete the archive, log, and every temporary file, then re-confirm the active worktree `HEAD` and status are unchanged.
