...
`LayoutRequest` carries the workspace ID (used for diagnostic labelling and as the primary seed namespace), current projection, stable-key diff, and pins. `GraphStreamLayoutEngine` retains particles for unaffected keys across requests, removes obsolete graph elements, and deterministically seeds new nodes and enclosure anchors from their stable keys. The seed for each particle is derived from SHA-256 over a canonical UTF-8 byte sequence prefixed by the workspace UUID. A projected node key encodes `node`, map UUID, and either persisted node-ID length plus bytes or the count and decimal values of structural-path indices. An enclosure hull key encodes `anchor`, the number of endpoint keys, and each endpoint source key in declared order using the same source-key encoding. The lower 64 digest bits seed `java.util.Random`. This encoding is injective for the supported key fields and stable across JVM restarts; it never uses `Object.hashCode()` or `toString()`, because `SourceNodeKey.toString()` intentionally omits persisted node IDs. Active pins set an exact particle position and freeze it; dormant pins do not create a particle or force. Pin coordinates (`PinProjection.x()` and `.y()`) are stored and interpreted in GraphStream physics layout space — the same coordinate space in which particle positions are reported in `LayoutPositions`. No coordinate-space conversion is applied.
...
`MapTierCorrection` is a pure operation over a projection, immutable positions, and hull geometry. It considers only non-suppressed map-root hull pairs, iterated in the order that those `ProjectedEnclosure` entries appear in `GraphProjection.enclosures()`, which is the stable projection output order. Each unordered pair `(A, B)` is visited once with `A` as the earlier enclosure; all pair translations are accumulated per map from the original hull snapshot and applied once at the end. Pairs are not revisited with roles reversed. A map is rigid if any active pin belongs to it. For an overlap:

- when both maps are movable, split `T = HullIntersection.minimumSeparatingTranslation(hullA, hullB)` uniformly: map A's particles translate by `-T/2` and map B's particles translate by `+T/2`, because `T` is the vector that moves the second hull B clear of the first hull A;
- when exactly one map is rigid, move the movable map by the complete vector: B receives `+T` when A is rigid, while A receives `-T` when B is rigid;
...
The correction translates all node particles and enclosure anchors belonging to a moved map by the same vector. It never translates only a root anchor or a subset of a pinned map. It is intentionally a transformation of each returned immutable frame: the Task 20 `LayoutEngine` boundary has no position-writeback operation, so Task 21 does not add one or leak GraphStream types. Because the engine has no writeback path, it always steps from its own internal uncorrected positions; correction is recomputed from scratch on every published frame, continuously nudging maps toward separation without disturbing the physics solver's internal energy bookkeeping. For every raw engine frame, `LayoutWorker` first computes temporary hull geometry from that frame's projection and positions, then applies `MapTierCorrection`, computes idle displacement against the previous corrected frame, and returns the corrected `LayoutFrame`. This works on the first frame without prior state. Task 23 later recomputes final hull geometry from each corrected frame before label placement and publication.
